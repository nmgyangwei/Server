...
SUCODEVersion CUCORENetWin::GetVersion(void)
{
...
ISDConnector* CUCORENetWin::CreateConnector(UINT32 dwNetIOType)
{
	...
	if(NULL == poConnector)
	{
		CRITICAL(_SDT("CUCORENetWin::CreateConnector, new CUCODEConnector failed"));
		return NULL;
	}
...
ISDListener* CUCORENetWin::CreateListener(UINT32 dwNetIOType)
{
    if (dwNetIOType & NETIO_COMPLETIONPORT)
    {
	    ...
	    if(NULL == poListener)
	    {
		    CRITICAL(_SDT("CUCORENetWin::CreateListener, new CUCODEListener failed"));
		    return NULL;
	    }
...
void CUCORENetWin::ReleaseConnector(CUCODEConnector* poConnector)
{
	if(NULL == poConnector)
	{
		return;
...
CUCODEConnector* CUCORENetWin::FindConnector(UINT32 dwID)
{
